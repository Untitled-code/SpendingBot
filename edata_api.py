...
def request(keyword, type, starDate, endDate, output_file):
    global totalAmount
    totalAmount = []
    print(keyword, type, starDate, endDate, output_file)
    OUTPUT_FILE = Path(output_file)
    """Adding parameters of the search depending on what keyword we need to search"""
    if type == "Розпорядник":
        PAYER = f'payers_edrpous={keyword}'
        RECIPT = ''
    elif type == "Отримувач":
        RECIPT = f'recipt_edrpous={keyword}'
        PAYER = ''
    REGION = ''
    """end"""
    ...
    for m in range(len(rangePairDates)):
        ...
        try:
            ...
            for i in range(len(resultDict)):
                # time.sleep(0.5)
                print(f'Entity:{keyword}, OUTER LOOP:{str(m)}, INNER:{str(i)}')
